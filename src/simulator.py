...
class LiveQuadrotorSimulator:
    ...
    def __init__(self, map_file=None):
        ...
        self.renderSettings = './vizflyt_viewer/render_settings/p2b.json'
        ...
        self.plots_dir = os.path.join(self.render_dir, 'plots')
        ...
        self.video_fps = 25
        self.video_path = os.path.join(self.plots_dir, "simulation.mp4")
        self._writer = None   
        

    def _video_start(self):
        if not self.record_video or self.fig is None:
            return
        try:
            self._writer = FFMpegWriter(fps=self.video_fps, metadata={"title": "Quad Sim"})
            self._writer.setup(self.fig, self.video_path, dpi=200)
            print(f"[video] recording -> {self.video_path}")
        except Exception as e:
            # fallback to GIF if ffmpeg not available
            self.video_path = self.video_path.replace(".mp4", ".gif")
            self._writer = PillowWriter(fps=self.video_fps)
            self._writer.setup(self.fig, self.video_path, dpi=150)
            print(f"[video] ffmpeg not found; recording GIF -> {self.video_path} ({e})")
    ...
    def _get_orientation_from_state(self):
        """Extract roll, pitch, yaw from quaternion state"""
        # State quaternion: [qx, qy, qz, qw] at indices 6-9
        qx, qy, qz, qw = self.state[6:10]
    
        # Convert to roll, pitch, yaw
        from splat_render import quaternion_to_euler
        roll, pitch, yaw = quaternion_to_euler(qx, qy, qz, qw)
        roll += np.pi  # flip about X
        
        return np.array([roll, pitch, yaw])

    def render_and_save_frame(self, frame_number):
        """Render current view and save RGB + depth images in separate folders"""
        try:
            # Get current position and orientation
            position = self.state[0:3]  # [x, y, z]
            # position[2] = -position[2]
            orientation_rpy = self._get_orientation_from_state()  # [roll, pitch, yaw]
            
            # Render the frame
            rgb, depth = self.renderer.render(position, orientation_rpy)
            
            # Save images to separate folders
            rgb_filename = os.path.join(self.rgb_dir, f'frame_{frame_number:05d}.png')
            depth_filename = os.path.join(self.depth_dir, f'frame_{frame_number:05d}.png')
            ...
            print(f"Warning: Rendering failed at frame {frame_number}: {e}")
            return False

# simulator.py

    def check_collision_during_flight(self):
        """
        Check if drone cuboid is in collision using current state
        Returns True if collision detected
        """
        # Get current position
        position = self.state[0:3]
        
        # Get orientation matrix from quaternion
        qx, qy, qz, qw = self.state[6:10]
        orientation_matrix = self._quaternion_to_rotation_matrix(qx, qy, qz, qw)
        
        # Check cuboid collision
        is_free = self.env.is_cuboid_in_free_space(position, orientation_matrix)
        
        return not is_free  # Return True if collision detected
    ...
    def initialize_execution_phase(self):
        ...
        self.state[6:10] = [1, 0, 0, 0]  # Identity quaternion
    ...
    def simulation_step(self):
        ...
        # === ADD RENDERING HERE ===
        # Render every N frames (e.g., every 5 frames to reduce overhead)
        if len(self.state_history) % 1 == 0:  
            frame_number = len(self.state_history) // 1
            self.render_and_save_frame(frame_number)
        # === END RENDERING ===
